...
## Overview and terminology[](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/#overview-and-terminology)
This page describes the deployment and release approach used to deliver changes to users. The overall process consists of two significant parts:
  1. **Monthly self-managed release** : GitLab version (XX.YY.0) [published every month](https://gitlab.com/gitlab-org/release/docs/blob/master/general/monthly/process.md). From this monthly release, [planned patches](https://handbook.gitlab.com/handbook/engineering/releases/patch-releases/) are scheduled twice a month and [unplanned critical patches](https://handbook.gitlab.com/handbook/engineering/releases/patch-releases/#patch-release-types) are created as needed.
...
For more details on the individual processes and how to use them please see the [Deployments page](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/deployments/) for GitLab.com changes and the [Releases page](https://handbook.gitlab.com/handbook/engineering/releases/) for changes for self-managed users.
**The main priority** of both deployments and releases **[is GitLab availability& security](https://handbook.gitlab.com/handbook/engineering/development/principles/#prioritizing-technical-decisions)** as an application running on both GitLab.com and for users running GitLab in their own infrastructure.
### Deployment and Release Process overview[](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/#deployment-and-release-process-overview)
...
![Deployment and Release process overview](https://handbook.gitlab.com/images/engineering/deployments-and-releases/deployment-and-release-process-overview.png)
  * [Diagram source](https://docs.google.com/presentation/d/1YRjA1dYCXNXp06VltDYlik1MdFyzUvaeXKk69mMPcA4/edit?usp=sharing)
  ...
  3. A scheduled pipeline packages all new changes into an “auto-deploy package” for deployment to GitLab.com. Multiple packages are created each day at the [listed times](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/deployments/#gitlabcom-deployments-process)
  ...
  5. The auto-deploy package is deployed to GitLab.com. For more details [see the deployment process](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/deployments/#gitlabcom-deployments-process)
...
For a more detailed explaination of the processes see the [deployments page](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/deployments/) and the [releases page](https://handbook.gitlab.com/handbook/engineering/releases/)
## Release Managers[](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/#release-managers)
...
### How to contact a Release Manager[](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/#how-to-contact-a-release-manager)
...
If you need to escalate a request, please use the [release management escalation process](https://handbook.gitlab.com/handbook/engineering/infrastructure-platforms/gitlab-delivery/delivery/#release-management-escalation)
### Weekly Delivery Metrics Review[](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/#weekly-delivery-metrics-review)
...
[MTTP Monthly](https://internal.gitlab.com/handbook/company/performance-indicators/product/saas-platforms-section/#mean-time-to-production-mttp) - [Deployment blockers](https://gitlab.com/groups/gitlab-com/gl-infra/-/work_items/1822) - [Deployment SLO](https://dashboards.gitlab.net/d/delivery-deployment_slo/delivery-deployment-slo?orgId=1) - [GitLab: deployment frequency and lead time](https://gitlab.com/gitlab-org/gitlab/-/analytics/dashboards/dora_metrics?date_range_option=30d)
  ...
  2. Walkthrough the monthly view of `Deployment frequency over time` and `Lead time for changes over time` panels in the [DORA metrics analytics dashboard](https://gitlab.com/gitlab-org/gitlab/-/analytics/dashboards/dora_metrics?date_range_option=30d) - Note any patterns or anomalies.
  3. Walkthrough the [Deployment SLO dashboard](https://dashboards.gitlab.net/d/delivery-deployment_slo/delivery3a-deployment-slo) - Deployment SLO and packager pipeline duration
  4. Walkthrough of [Deployment Blockers Dashboard](https://dashboards.gitlab.net/d/delivery-deployment_blockers/delivery3a-deployment-blockers?orgId=1&from=now-6d&to=now&timezone=utc&var-PROMETHEUS_DS=mimir-gitlab-ops&var-root_cause=$__all) and last week’s [Deployment Blockers](https://gitlab.com/groups/gitlab-com/gl-infra/-/epics/1496)
  5. Do we need to take action based on the previous week’s MTTP?


### Deployment Targets[](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/#deployment-targets)
...
#### Taxonomy[](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/#taxonomy)
...
GitLab.com is our original monolithic deployment, configured via the [config-mgmt](https://ops.gitlab.net/gitlab-com/gl-infra/config-mgmt/) project. Primarily uses CNG charts, and uses Omnibus for deploying Gitaly, Patroni and Postgres. No service ping data is sent.
...
Dedicated is our _single-tenant_ SaaS offering. Dedicated primarily uses GitLab Environment Toolkit (GET) Hybrid. In turn, GET relies on Omnibus (for Gitaly) and Cloud Native Charts (CNG) for application services. Where possible Cloud-Managed services are favoured, including Postgres (RDS, CloudSQL), Redis (ElastiCache, MemoryStore) and Kubernetes (EKS, GKE).
...
Autonomous System (ASN) | Amazon  
      2. **US Public Sector** `~deployment-type:saas-dedicated-uspubsec`
...
GitLab Environment Toolkit targets cloud environments in AWS, Google Cloud and Microsoft Azure. All GET deployment targets are included in the [reference architectures](https://docs.gitlab.com/administration/reference_architectures/), providing guidelines for number of nodes, and appropriate sizing of nodes.
...
On-Premise (“on-prem”) in this document refers to non-cloud-based deployments, _or_ deployments to clouds other than AWS, Google Cloud or Azure. Generally, on-premise deployments are unable to use any managed services for Postgres, Redis, Kubernetes, etc. Historically, Omnibus has provided a means of configuring these services, or users can bring their own. For some proposed new services, such as Secrets Management, on-premise brings unique challenges in that PKCS11-compatible hardware may need to be integrated due to the lack of cloud KMS.
...
Customer must build their own configuration tooling to deploy GitLab (e.g. Ansible, Chef, Saltstack, etc). We encourage customers to follow the [reference architectures](https://docs.gitlab.com/administration/reference_architectures/) to choose the number of nodes and sizes of nodes.
...
## Resources[](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/#resources)
...
Deployment process | [Link](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/deployments/)  
Release process | [Link](https://handbook.gitlab.com/handbook/engineering/releases/)  
...
Overview and terminology This page describes the individual steps involved in deploying application …
Last modified January 12, 2026: [Update deployment blockers link in deployments and releases handbook (`0d185086`)](https://gitlab.com/gitlab-com/content-sites/handbook/commit/0d185086)
[ ](https://gitlab.com/gitlab-com/content-sites/handbook/blob/main/content/handbook/engineering/deployments-and-releases/_index.md) - [ ](https://gitlab.com/-/ide/project/gitlab-com/content-sites/handbook/edit/main/-/content/handbook/engineering/deployments-and-releases/_index.md) - please [contribute](https://handbook.gitlab.com/handbook/about/contributing/). [ ![Creative Commons License](https://i.creativecommons.org/l/by-sa/4.0/80x15.png)](https://creativecommons.org/licenses/by-sa/4.0/)