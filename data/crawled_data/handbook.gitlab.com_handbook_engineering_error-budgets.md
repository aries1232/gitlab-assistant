...
As part of [our strategy](https://about.gitlab.com/direction/#3-year-strategy) to [reinforce GitLab SaaS](https://about.gitlab.com/direction/core_platform/dotcom/) as an enterprise grade platform ready for business critical workloads, GitLab.com has specific [Availability](https://handbook.gitlab.com/handbook/engineering/infrastructure/performance-indicators/#gitlabcom-availability) and [Performance](https://handbook.gitlab.com/handbook/engineering/infrastructure/performance-indicators/#gitlabcom-performance) targets.
These targets give our users indication of the platform reliability.
Additionally, [GitLab.com Service Level Availability](https://handbook.gitlab.com/handbook/engineering/monitoring/#gitlabcom-service-availability) is also a part of our contractual agreement with platform customers. The contract might define a specific target number, and not honouring that agreement may result in financial and reputational burdens.
## What are error budgets?[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#what-are-error-budgets)
...
## What are the components of error budgets?[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#what-are-the-components-of-error-budgets)
...
Taken all together, the above example SLO would be: _**99.95% of the 95th percentile latency of api requests over 5 mins is < 100ms over the previous 28 days**_
The Error Budget is then 1 - Objective of the SLO, in this case (1 - .9995 = .0005). Using our 28 day timeframe, **the “budget” for errors is 20.16 minutes** (.0005 _(28_ 24 * 60))
...
GitLab’s current implementation of Error Budgets is only using some of the above sophistication of SLOs and Error Budgets, but we expect to increase the sophistication in the future. It is expected that the practices of SLOs and Error Budgets evolve to have _**both**_ the objective and the SLI vary (appropriately) based on the criticality of the service as well as the resiliency of other services and components which depend on it.
## Which types of errors are included?[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#which-types-of-errors-are-included)
...
## Why are we using error budgets?[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#why-are-we-using-error-budgets)
...
The [Infradev Process](https://handbook.gitlab.com/handbook/engineering/workflow/#infradev) was created to prioritize resolving an issue after an incident or degradation has happened. While the process has proven to be successful, it is _event-focused_ and _event-driven_.
The [Engineering Allocation Process](https://handbook.gitlab.com/handbook/product/product-processes/#prioritization-framework) was created to address long term team efficiency, performance and security items.
...
## How do we determine the highest priority improvements?[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#how-do-we-determine-the-highest-priority-improvements)
Each group has a `Budget spend attribution` section in their [Budget detail dashboard](https://docs.gitlab.com/ee/development/stage_group_observability/dashboards/error_budget_detail.html) that allows them to [discover where their budget is being spent](https://docs.gitlab.com/ee/development/stage_group_observability/index.html#check-where-budget-is-being-spent).
...
Even though `Endpoint A` has a lower apdex ratio, fixing more violations from `Endpoint B` will have a greater impact on the error budget overall, as they account for the majority of violations impacting the error budget.
Now let’s take a look at a more complex violation scenario:
Endpoint | Total requests | Slow requests | Apdex ratio | Traffic share  
---|---|---|---|---  
Endpoint A | 100 000 | 30 | 99.97% | 80%  
Endpoint B | 25 000 | 30 | 99.88% | 20%  
The 30 errors from `Endpoint A` and `Endpoint B` both have an identical impact on the error budget. However, it would be better to work on `Endpoint B` because it has a higher ratio of slow requests / overall requests. Because `Endpoint A` is already meeting the target of 99.95%, no additional work is required here. 99.97% is a good score and any deliberate improvements here could be considered a premature optimization.
The number of violations for `Endpoint B` puts it below the apdex threshold, so if these two endpoints are the top violators we see, we should look into improving `Endpoint B`.
## The Error Budget Policy for GitLab.com[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#the-error-budget-policy-for-gitlabcom)
The error budgets process has a few distinct items:
  1. Budget stakeholders
  2. Budget allocation
  3. Budget spend and accounting
  4. Communication between the stakeholders


## Budget stakeholders[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#budget-stakeholders)
The stakeholders in the Error Budget process are:
  1. Stage teams (Product department and the supporting Engineering teams represented on the [product categories page](https://handbook.gitlab.com/handbook/product/categories/))
  2. Infrastructure teams (Teams represented on the [infrastructure team page](https://handbook.gitlab.com/handbook/engineering/infrastructure/team/))
  3. [VP of Infrastructure and Infrastructure Leadership](https://handbook.gitlab.com/handbook/engineering/infrastructure/#mstaff)
  4. VP of Development and VP of Product


## Budget allocation[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#budget-allocation)
Error budget is calculated based on the [availability](https://handbook.gitlab.com/handbook/engineering/infrastructure/performance-indicators/#gitlabcom-availability) targets.
...
The budget is set on the SaaS platform and is shared between stage and infrastructure teams. Service Availability calculation methodology is covered in details at [the GitLab.com Service Availability page](https://handbook.gitlab.com/handbook/engineering/monitoring/#gitlabcom-service-availability).
...
## Budget Reporting[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#budget-reporting)
On the 4th of each month, the Error Budget Report is delivered for the Budget Spend by Stage Group. The announcements appear in `#product`, `#eng-managers`, `#f_error_budgets` and `#development`.
...
### Error Budgets in the Engineering Allocation Meeting[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#error-budgets-in-the-engineering-allocation-meeting)
...
## Budget spend(by service)[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#budget-spendby-service)
...
Spent budget is the time (in minutes) during which user facing services have experienced a percentage of errors below the specified threshold and latency is above the specified objectives for the service. The details on how Service Availability is calculated can be found at [the GitLab.com Service Availability page](https://handbook.gitlab.com/handbook/engineering/monitoring/#gitlabcom-service-availability).
The budget spend is currently aggregated at the primary service level.
![Complete budget](https://handbook.gitlab.com/images/engineering/error-budgets/complete-budget.png)
![Spent budget](https://handbook.gitlab.com/images/engineering/error-budgets/spent-budget.png)
...
## Budget spend (by stage group)[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#budget-spend-by-stage-group)
There is [an example available](https://handbook.gitlab.com/handbook/engineering/error-budgets/error-budget-by-stage-group-example/) with a more detailed look at how this is built.
The current [28 day](https://handbook.gitlab.com/handbook/enterprise-data/organization/programs/data-for-product-managers/) budget spend can be found on each [stage group dashboard](https://dashboards.gitlab.net/dashboards/f/stage-groups/stage-groups). Feature categories for that stage group are rolled up to a single value.
...
Apdex and Error Rates are explained in more detail on [the handbook page](https://handbook.gitlab.com/handbook/engineering/monitoring/#gitlabcom-service-level-availability).
...
### System-wide incidents[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#system-wide-incidents)
...
### How to change error budget attribution[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#how-to-change-error-budget-attribution)
...
### Contract[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#contract)
...
Error Budgets should be reviewed monthly as part of the [Product Development Timeline](https://handbook.gitlab.com/handbook/engineering/workflow/#product-development-timeline).
...
> 20 minutes | Commitment to [reliability/availability improvements](https://handbook.gitlab.com/handbook/product/product-processes/#prioritization), feature development is secondary.  
...
#### Stage Groups with different error budgets[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#stage-groups-with-different-error-budgets)
...
Valid reasons for an exception are:
  ...
  2. Work for improving the error budget is scoped out and fully planned, but the work is not currently funded. The stakeholders are in the process of securing the funding, and the Error Budget will be regularly spent until the additional funds are secured.
  3. Temporarily, the highest priority is to achieve a significant business goal, and the reliability of GitLab.com is not directly affected. The group is likely to regularly spend the Error Budget while they are focused on this other priority.
...
To request an exception, open an MR and add the stage group to the table above. In the description, supply the following details:
  ...
  2. Relevant resources showing that the work is scoped out
  3. Target date when the exception must be revisited


**Additional Guidance**
  1. Document the work to be done using Epics and Issues
  2. Add detailed descriptions to the Epics and Issues to ensure the work is clearly scoped out.
  3. Add a start date and due date to the Epic so that it is transparent how long it will take to complete the work.
  4. Add milestones to the issues so it is transparent when the work will be planned.
...
Provide answers to the following questions:
  ...
  2. What is the main contributor to your team’s error budget spend? Is that the response time?
  3. What does success look like at the closure of referenced epic?
...
**Assign the MR for approval to:**
  ...
  2. Senior Manager of Infrastructure or up 
...
### Error Budget Improvements[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#error-budget-improvements)
Work relating to error budget improvements should be detailed in an issue. Please label these issues with `Error Budget Improvement` and the `group::` label so they can be tracked in reports.
### Error Budget DRIs[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#error-budget-dris)
...
Infrastructure | [Setting the Error Budget Minutes and Availability Target](https://handbook.gitlab.com/handbook/engineering/infrastructure/performance-indicators/#gitlabcom-availability) | 99.95% (20 minutes over 28 days Error Budget) | Complete - In Grafana  
  * For groups with [engineering allocations](https://handbook.gitlab.com/handbook/product/product-processes/#prioritization-framework), the responsibility to maintain the spend of error budget is with the development team instead of the product management team.


## Current State and Future Intent[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#current-state-and-future-intent)
### Current State[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#current-state)
  1. Error budgets exist for each feature category and incorporate a standard apdex threshold and error rate.
  2. Error budgets are published for stage groups and stages through Grafana and Sisense Dashboards.
  3. Contributing factors are explorable through links available on the Grafana Dashboards.
  4. Error budgets are included in the [Product Prioritization process](https://handbook.gitlab.com/handbook/engineering/workflow/#product-development-timeline).


### Roadmap[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#roadmap)
The changes below aim to increase the maturity of the Error Budgets.
#### 1. Increase precision of Error Budget calculations (apdex portion)[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#1-increase-precision-of-error-budget-calculations-apdex-portion)
**Improvements**
...
Product Development teams are encouraged to:
  * Continue working on [Strategic Priority Codes](https://handbook.gitlab.com/handbook/engineering/workflow/strategic-priority-codes/), [Infradev](https://handbook.gitlab.com/handbook/engineering/workflow/#availability-and-performance-refinement), [Corrective Actions](https://handbook.gitlab.com/handbook/engineering/infrastructure-platforms/incident-review/), [Security](https://handbook.gitlab.com/handbook/security/product-security/vulnerability-management/#vulnerability-management-overview), and [Engineering Allocation](https://handbook.gitlab.com/handbook/product/product-processes/#prioritization-framework) issues per our [Prioritization guidelines](https://handbook.gitlab.com/handbook/engineering/development/principles/#prioritizing-technical-decisions)
...
#### 2. Increase visibility into Error Budgets (error portion)[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#2-increase-visibility-into-error-budgets-error-portion)
...
#### 3. Tune the scope of Error Budgets[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#3-tune-the-scope-of-error-budgets)
  * Consider incorporating P1/S1 incidents into the Error Budget Calculation.


## More information[](https://handbook.gitlab.com/handbook/engineering/error-budgets/#more-information)
...
Engineering Error Budget by stage groups example.
...
[ ](https://gitlab.com/gitlab-com/content-sites/handbook/blob/main/content/handbook/engineering/error-budgets/_index.md) - [ ](https://gitlab.com/-/ide/project/gitlab-com/content-sites/handbook/edit/main/-/content/handbook/engineering/error-budgets/_index.md) - please [contribute](https://handbook.gitlab.com/handbook/about/contributing/). [ ![Creative Commons License](https://i.creativecommons.org/l/by-sa/4.0/80x15.png)](https://creativecommons.org/licenses/by-sa/4.0/)