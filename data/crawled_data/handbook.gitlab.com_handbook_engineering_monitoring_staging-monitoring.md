...
[Staging environment](https://handbook.gitlab.com/handbook/engineering/infrastructure-platforms/environments/#staging) has the same [service-level monitoring rules as Production](https://handbook.gitlab.com/handbook/engineering/):
...
>   2. _AND_ The [Error rate](https://en.wikipedia.org/wiki/Bit_error_rate) is _below_ its Service Level Objective ([SLO](https://en.wikipedia.org/wiki/Service-level_objective)).
> 
...
Staging environment doesn’t have as much user activity as Production since it doesn’t have the same amount of real users. The environment is mostly used by test automation like [GitLab QA pipelines](https://handbook.gitlab.com/handbook/engineering/testing/end-to-end-pipeline-monitoring/) and engineers who may test their code manually. These activities don’t generate enough traffic so a custom load [emulation tool](https://handbook.gitlab.com/handbook/engineering/monitoring/staging-monitoring/#load-emulation) was designed to create artificial traffic to compensate for the lack of signal from real users.
## Load emulation[](https://handbook.gitlab.com/handbook/engineering/monitoring/staging-monitoring/#load-emulation)
...
Known limitations:
  ...
  * Git traffic is HTTP-only [at the moment](https://handbook.gitlab.com/handbook/engineering/monitoring/staging-monitoring/#ongoing-work).


### Crawler configuration on Staging[](https://handbook.gitlab.com/handbook/engineering/monitoring/staging-monitoring/#crawler-configuration-on-staging)
...
Throughput is controlled by environment variables and tuned differently for each service. It’s worth noting that if load needs to be increased, Staging environment’s performance should be considered and verified that it doesn’t affect existing [GitLab QA pipelines](https://handbook.gitlab.com/handbook/engineering/testing/end-to-end-pipeline-monitoring/). Otherwise, it may bring intermittent errors in the test runs and affect the deployment.
## Staging Service Monitoring[](https://handbook.gitlab.com/handbook/engineering/monitoring/staging-monitoring/#staging-service-monitoring)
...
More detailed information about service monitoring can be found on [Monitoring of GitLab.com](https://handbook.gitlab.com/handbook/engineering/) page.
...
## Ongoing Work[](https://handbook.gitlab.com/handbook/engineering/monitoring/staging-monitoring/#ongoing-work)
...
[ ](https://gitlab.com/gitlab-com/content-sites/handbook/blob/main/content/handbook/engineering/monitoring/staging-monitoring.md) - [ ](https://gitlab.com/-/ide/project/gitlab-com/content-sites/handbook/edit/main/-/content/handbook/engineering/monitoring/staging-monitoring.md) - please [contribute](https://handbook.gitlab.com/handbook/about/contributing/). [ ![Creative Commons License](https://i.creativecommons.org/l/by-sa/4.0/80x15.png)](https://creativecommons.org/licenses/by-sa/4.0/)