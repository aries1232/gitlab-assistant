...
This document explains the workflow for anyone working with issues in GitLab Inc. For the workflow that applies to the wider community see the [contributing guide](https://docs.gitlab.com/ee/development/contributing/).
## GitLab Flow[](https://handbook.gitlab.com/handbook/engineering/workflow/#gitlab-flow)
Products at GitLab are built using the [GitLab Flow](https://about.gitlab.com/blog/2023/07/27/gitlab-flow-duo/).
We have [specific rules around code review](https://handbook.gitlab.com/handbook/engineering/workflow/code-review/).
## Reverting a merge request[](https://handbook.gitlab.com/handbook/engineering/workflow/#reverting-a-merge-request)
In line with our values of [short toes](https://handbook.gitlab.com/handbook/values/#short-toes), [making two-way-door decisions](https://handbook.gitlab.com/handbook/values/#make-two-way-door-decisions) and [bias for action](https://handbook.gitlab.com/handbook/values/#operate-with-a-bias-for-action), anyone can propose to revert a merge request. When deciding whether an MR should be reverted, the following should be true:
  * Something broke and there is no acceptable work around. Examples of this include: 
    * A feature broke and is categorized as `~severity::1` or `~severity::2`. [See severity labels](https://handbook.gitlab.com/handbook/product-development/how-we-work/issue-triage/#severity)
    * [Master broken](https://handbook.gitlab.com/handbook/engineering/workflow/#broken-master)
...
## Broken `master`[](https://handbook.gitlab.com/handbook/engineering/workflow/#broken-master)
...
### What is a broken `master`?[](https://handbook.gitlab.com/handbook/engineering/workflow/#what-is-a-broken-master)
...
### Broken `master` service level objectives[](https://handbook.gitlab.com/handbook/engineering/workflow/#broken-master-service-level-objectives)
...
---|---|---  
[Triage](https://handbook.gitlab.com/handbook/engineering/workflow/#triage-broken-master) | 4 hours from the 2nd occurrence of a broken `master` incident creation until assignment | group labeled on the incident  
[Resolution](https://handbook.gitlab.com/handbook/engineering/workflow/#resolution-of-broken-master) | 4 hours from assignment to DRI until incident is resolved | Merge request author or team of merge request author or dev on-call engineer  
...
```
Pipeline   failure   incident   #1not   recurring   in   24hours   and   no   humanactivityAuto   closedany   human   updateon   incidentlabels   incident   #1with   escalationskippeddoes   not   trigger   anygroup   ping   orescalationsame   job   failuresrecurring   in   incidentcloses   incident   #2   asduplicate   of   incident#1labels   incident   #1with   escalationneededpings   attributedgroup   channel   after10   minutes   ofinactivity2nd   ping   to   groupchannel   after   30minutes   of   inactivitypings   stage   channelin   after   3   hours   40minutes   of   inactivityescalates   todevelopment   after   4hours   of   inactivitylabels   incident   #1   isescalatedPipeline incident escalation
```

If an incident becomes a blocker for MRs and deployments before being auto-escalated, the team member being impacted should refer to the [broken `master` escalation](https://handbook.gitlab.com/handbook/engineering/workflow/#broken-master-escalation) steps to request help from the current [engineer on-call](https://handbook.gitlab.com/handbook/engineering/infrastructure-platforms/incident-management/#who-is-the-current-eoc) as early as needed.
Additional details about the phases are listed below.
### Broken `master` escalation[](https://handbook.gitlab.com/handbook/engineering/workflow/#broken-master-escalation)
...
If a broken `master` is blocking your team before auto-escalation (such as creating a security release) then you should:
  ...
  2. Check discussions on the failure notification in the triage DRI’s group Slack channel to see if anyone is investigating the incident you are looking at. See [Triage broken master](https://handbook.gitlab.com/handbook/engineering/workflow/#triage-broken-master) for information on who the triage DRI is.
  3. If there is not a clear DRI or action to resolve, use the [dev escalation](https://handbook.gitlab.com/handbook/engineering/workflow/development-processes/infra-dev-escalation/process/) process to solicit help in the broken `master` incident.


**Note:** Stable branch failures follow the same process as described here, but incidents are tracked in [gitlab-org/release/tasks](https://gitlab.com/gitlab-org/release/tasks). See [stable branches documentation](https://handbook.gitlab.com/handbook/engineering/releases/stable_branches/#broken-stable-branches) for details.
#### Escalation on weekends and holidays[](https://handbook.gitlab.com/handbook/engineering/workflow/#escalation-on-weekends-and-holidays)
Master broken incidents must be manually escalated to `#development` if necessary. Without a manual escalation, the service level objective can extend to the next working day; that is, triage DRI is expected to triage the incident on the next working day. Regardless of when the label was applied, we always consider an incident to be in an `escalated` state as long as it has the ~" escalation::escalated" label, until the incident is resolved.
### Triage broken master[](https://handbook.gitlab.com/handbook/engineering/workflow/#triage-broken-master)
#### Definitions[](https://handbook.gitlab.com/handbook/engineering/workflow/#definitions)
...
#### Attribution[](https://handbook.gitlab.com/handbook/engineering/workflow/#attribution)
...
#### Triage DRI Responsibilities[](https://handbook.gitlab.com/handbook/engineering/workflow/#triage-dri-responsibilities)
  1. Monitor
...
```
  ...
  2. Identification
     ...
     * If this incident is **due to non-flaky reasons** , communicate in `#development`, `#backend`, and `#frontend` using the Slack Workflow.
       ...
       * [Create a revert MR directly](https://handbook.gitlab.com/handbook/engineering/workflow/#reverting-a-merge-request) to save some time in case we need to revert down the line. 
         * If you are reverting an MR that performs a database migration, you need to follow the [Deployment blockers process](https://handbook.gitlab.com/handbook/engineering/deployments-and-releases/deployments/#deployment-blockers) to prevent the migration from proceeding to deploy and running on staging and production.
...
The triage DRI can create a merge request, assign to any available maintainer, and ping the resolution DRI with a `@username FYI` message. Additionally, a message can be posted in `#backend_maintainers` or `#frontend_maintainers` to get a maintainer take a look at the fix ASAP.
...
#### Pro-tips for Triage DRI[](https://handbook.gitlab.com/handbook/engineering/workflow/#pro-tips-for-triage-dri)
...
### Resolution of broken master[](https://handbook.gitlab.com/handbook/engineering/workflow/#resolution-of-broken-master)
...
#### Responsibilities of the resolution DRI[](https://handbook.gitlab.com/handbook/engineering/workflow/#responsibilities-of-the-resolution-dri)
  1. Prioritize resolving recurring broken `master` incidents over new bug/feature work. Resolution options include: 
     * **Default** : Revert the merge request which caused the broken `master`. If a revert is performed, create an issue to reinstate the merge request and assign it to the author of the reverted merge request. 
  ...
  4. Backport the broken `master` incident to the [maintained stable branches](https://docs.gitlab.com/policy/maintenance/#maintained-versions). See [stable branches](https://handbook.gitlab.com/handbook/engineering/workflow/#stable-branches)
  ...
  8. Create an [issue](https://gitlab.com/gitlab-org/quality/analytics/team/-/issues/new) for the [Development Analytics group](https://handbook.gitlab.com/handbook/engineering/infrastructure-platforms/developer-experience/development-analytics/) describing how the broken `master` incident could have been prevented in the Merge Request pipeline.
  9. When resolution steps are completed and all of the required fixes are merged, close the incident.


#### Responsibilities of authors and maintainers[](https://handbook.gitlab.com/handbook/engineering/workflow/#responsibilities-of-authors-and-maintainers)
Once the resolution DRI announces that `master` is fixed:
  * Maintainers should start a new merged results pipeline (for canonical MRs) and enable “Auto-merge”. There’s no need to rebase once `master` has been fixed since we use [merged results pipelines](https://docs.gitlab.com/ee/ci/pipelines/merged_results_pipelines.html).
  * (For forks only) Authors should rebase their open merge requests (since [merged results pipelines](https://docs.gitlab.com/ee/ci/pipelines/merged_results_pipelines.html) isn’t supported in these cases).


### Merging during broken master[](https://handbook.gitlab.com/handbook/engineering/workflow/#merging-during-broken-master)
...
In the rare case where a merge request is [urgent](https://handbook.gitlab.com/handbook/engineering/workflow/#criteria-for-merging-during-broken-master) and must be merged **immediately** , team members can follow the process below to have a merge request merged during a broken `master`.
#### Criteria for merging during broken master[](https://handbook.gitlab.com/handbook/engineering/workflow/#criteria-for-merging-during-broken-master)
...
#### How to request a merge during a broken `master`[](https://handbook.gitlab.com/handbook/engineering/workflow/#how-to-request-a-merge-during-a-broken-master)
First, ensure the latest pipeline has completed less than 2 hours ago (although it is likely to have failed due to `gitlab-org/gitlab` using [merged results pipelines](https://docs.gitlab.com/ee/ci/pipelines/merged_results_pipelines.html)).
Next, make a request on Slack:
  1. Post to either the `#frontend_maintainers` _or_ `#backend_maintainers` Slack channels (whichever one is more relevant).
  2. In your post outline _why_ the merge request is [urgent](https://handbook.gitlab.com/handbook/engineering/workflow/#criteria-for-merging-during-broken-master).
  3. Make it clear that this would be a merge during a broken `master`, optionally add a link to this page in your request.


#### Instructions for the maintainer[](https://handbook.gitlab.com/handbook/engineering/workflow/#instructions-for-the-maintainer)
A maintainer who sees a request to merge during a broken `master` must follow this process.
Note, if any part of the process below disqualifies a merge request from being merged during a broken `master` then the maintainer must inform the requestor as to _why_ in the merge request (and optionally in the Slack thread of the request).
First, assess the request:
  ...
  2. Assess whether the merge request [is urgent or not](https://handbook.gitlab.com/handbook/engineering/workflow/#criteria-for-merging-during-broken-master). If in doubt, ask the requestor for more details in the merge request about why it is urgent.


Next, ensure that all the following conditions are met:
  1. The latest pipeline has completed less than 2 hours ago (although it is likely to have failed due to `gitlab-org/gitlab` using [merged results pipelines](https://docs.gitlab.com/ee/ci/pipelines/merged_results_pipelines.html)).
  2. All of the latest pipeline failures also happen on `master`.
  3. There is a corresponding non-resolved [broken `master` incidents](https://gitlab.com/gitlab-org/quality/engineering-productivity/master-broken-incidents/-/issues). See the “Triage DRI Responsibilities” steps above for more details.
...
Next, merge the merge request:
  ...
  * Otherwise, you must: 
    1. Unset the [“Pipelines must succeed” setting](https://docs.gitlab.com/ee/user/project/merge_requests/auto_merge.html#require-a-successful-pipeline-for-merge) for the [`gitlab-org/gitlab` project](https://gitlab.com/gitlab-org/gitlab/edit).
    2. Click the “Merge” button.
    3. If the merge train is enabled, a warning will be displayed stating the code changes won’t be validated by the merge train. Considering the criticality of the merge request it is acceptable to dismiss the warning.
    4. Set the “Pipelines must succeed” setting to be on again.


### Broken `master` mirrors[](https://handbook.gitlab.com/handbook/engineering/workflow/#broken-master-mirrors)
[`#master-broken-mirrors`](https://gitlab.slack.com/archives/C01PK38VAN8) was created to remove duplicative notifications from the `#master-broken` channel which provides a space for [Release Managers](https://about.gitlab.com/community/release-managers/) and the [Developer Experience teams](https://handbook.gitlab.com/handbook/engineering/infrastructure-platforms/developer-experience/) to monitor failures for the following projects:
...
### Broken JiHu validation pipelines[](https://handbook.gitlab.com/handbook/engineering/workflow/#broken-jihu-validation-pipelines)
We run JiHu validation pipelines in some of the merge requests, and it can be broken at times. When this happens, check [What to do when the validation pipeline failed](https://handbook.gitlab.com/handbook/finance/jihu-support/jihu-validation-pipelines/#what-to-do-when-the-validation-pipeline-failed) for more details.
## Stable branches[](https://handbook.gitlab.com/handbook/engineering/workflow/#stable-branches)
...
Security issues are managed and prioritized by the security team. If you are assigned to work on a security issue in a milestone, you need to follow the [Security Release process](https://gitlab.com/gitlab-org/release/docs/blob/master/general/security/process.md).
...
For more information on how the entire process works for security releases, see the [documentation on security releases](https://gitlab.com/gitlab-org/release/docs/blob/master/general/security/process.md).
## Regressions[](https://handbook.gitlab.com/handbook/engineering/workflow/#regressions)
...
### Use of the ~regression label on MRs[](https://handbook.gitlab.com/handbook/engineering/workflow/#use-of-the-regression-label-on-mrs)
...
## Basics[](https://handbook.gitlab.com/handbook/engineering/workflow/#basics)
  ...
  2. If you need to schedule something or prioritize it, apply the appropriate labels (see [Scheduling issues](https://handbook.gitlab.com/handbook/engineering/workflow/#scheduling-issues)).
  ...
  7. You (and your team, if applicable) are responsible for:
     ...
     * Shipping secure code, (see [Security is everyone’s responsibility](https://handbook.gitlab.com/handbook/engineering/workflow/#security-is-everyones-responsibility)).
...
## Updating Workflow Labels Throughout Development[](https://handbook.gitlab.com/handbook/engineering/workflow/#updating-workflow-labels-throughout-development)
...
Workflow labels are described in our [Development Documentation](https://gitlab.com/gitlab-org/gitlab-foss/-/blob/master/doc/development/labels/index.md#workflow-labels) and [Product Development Flow](https://handbook.gitlab.com/handbook/product-development/how-we-work/product-development-flow/).
## Working in Teams[](https://handbook.gitlab.com/handbook/engineering/workflow/#working-in-teams)
For larger issues or issues that contain many different moving parts, you’ll be likely working in a team. This team will typically consist of a [backend engineer](https://handbook.gitlab.com/job-description-library/engineering/backend-engineer/), a [frontend engineer](https://handbook.gitlab.com/job-description-library/engineering/development/frontend/), a [Product Designer](https://handbook.gitlab.com/job-description-library/product/product-designer/) and a [product manager](https://handbook.gitlab.com/job-description-library/product/product-manager/).
...
In the spirit of [collaboration](https://handbook.gitlab.com/handbook/values/#collaboration) and [efficiency](https://handbook.gitlab.com/handbook/values/#efficiency-for-the-right-group), members of teams should feel free to discuss issues directly with one another while [being respectful of others’ time](https://handbook.gitlab.com/handbook/communication/#be-respectful-of-others-time).
## Convention over Configuration[](https://handbook.gitlab.com/handbook/engineering/workflow/#convention-over-configuration)
Avoid adding configuration values in the application settings or in `gitlab.yml`. Only add configuration if it is absolutely necessary. If you find yourself adding parameters to tune specific features, stop and consider how this can be avoided. Are the values really necessary? Could constants be used that work across the board? Could values be determined automatically? See [Convention over Configuration](https://handbook.gitlab.com/handbook/product/product-principles/#convention-over-configuration) for more discussion.
## Choosing Something to Work On[](https://handbook.gitlab.com/handbook/engineering/workflow/#choosing-something-to-work-on)
...
## Triaging and Reviewing Code from the rest of the Community[](https://handbook.gitlab.com/handbook/engineering/workflow/#triaging-and-reviewing-code-from-the-rest-of-the-community)
It’s every [developers’ responsibilities](https://handbook.gitlab.com/job-description-library/engineering/backend-engineer/#responsibilities) to triage and review code contributed by the rest of the community, and work with them to get it ready for production.
...
## Working with GitLab.com[](https://handbook.gitlab.com/handbook/engineering/workflow/#working-with-gitlabcom)
...
### Performance Data[](https://handbook.gitlab.com/handbook/engineering/workflow/#performance-data)
There is extensive [monitoring](https://dashboards.gitlab.com/) publicly available for GitLab.com. For more on this and related tools, see the [monitoring handbook](https://handbook.gitlab.com/handbook/engineering/monitoring/).
### Error Reporting[](https://handbook.gitlab.com/handbook/engineering/workflow/#error-reporting)
  ...
  * [prometheus.gitlab.com](https://prometheus.gitlab.com/alerts) has alerts for the [production team](https://handbook.gitlab.com/handbook/engineering/infrastructure/#production-team)


## Scheduling Issues[](https://handbook.gitlab.com/handbook/engineering/workflow/#scheduling-issues)
...
Product Managers are responsible for scheduling all issues in their respective [product areas](https://handbook.gitlab.com/handbook/product/categories/#devops-stages), including features, bugs, and tech debt. Product managers alone determine the [prioritization](https://handbook.gitlab.com/handbook/product/product-processes/cross-functional-prioritization/), but others are encouraged to influence the PMs decisions. The UX Lead and Engineering Leads are responsible for allocating people making sure things are done on time. Product Managers are _not_ responsible for these activities, they are not project managers.
...
### Requesting Something to be Scheduled[](https://handbook.gitlab.com/handbook/engineering/workflow/#requesting-something-to-be-scheduled)
To request scheduling an issue, ask the [responsible product manager](https://handbook.gitlab.com/handbook/product/categories/#devops-stages)
...
[![’’](https://handbook.gitlab.com/images/engineering/workflow/gitlab-release-timelines.png)](https://gitlab.com/gitlab-org/gitlab/-/snippets/3670861)
...
With the monthly [release date](https://handbook.gitlab.com/handbook/engineering/releases/) being the third Thursday of the release month, the **code cut-off** is the Friday prior.
...
All other important dates for a milestone are relative to the release date:
  * **Monday, 19 days before the milestone begins** : 
    ...
    * Development engineering manager provides product manager with prioritization input for `~type::maintenance` issues per [cross-functional prioritization](https://handbook.gitlab.com/handbook/engineering/workflow/cross-functional-prioritization/)
    * [Quality](https://handbook.gitlab.com/handbook/engineering/workflow/cross-functional-prioritization/) provides product manager with prioritization input for `~type::bug` issues per [cross-functional prioritization](https://handbook.gitlab.com/handbook/engineering/workflow/cross-functional-prioritization/)
  ...
  * **Monday, 5 days before the milestone begins** : 
    * Release scope is finalized. In-scope issues marked with milestone `%x.y`; label `~deliverable` applied.
    * Kickoff document is updated with relevant items to be included.
  * **Monday immediately after the milestone begins** : _**Kick off!**_ 📣 
    * [Company Kickoff](https://handbook.gitlab.com/handbook/engineering/workflow/#kickoff) call live streamed.
    * Development on milestone begins.
  * **Monday, 9 days after the milestone begins** : 
    * The development lead for each stage/section coordinates a stage/section level review with the quad [cross-functional dashboard review process](https://handbook.gitlab.com/handbook/product/product-processes/cross-functional-prioritization/#cross-functional-dashboard-reviews). After the stages/section level reviews are complete, the VP of Development coordinates a summary review with the CTO, VP of Product, VP of UX, and VP of Quality.
  * **Wednesday, 11 days after the milestone begins** : 
    * GitLab Bot opens [Group Retrospective](https://handbook.gitlab.com/handbook/engineering/careers/management/group-retrospectives/) issue for the current milestone.
  * **Friday, the day the milestone ends** : 
    * Issues for milestone completed, with docs, and have been merged into master.
    * Feature flags should be flipped from default off to default on after verification to be in the release. See [feature flags](https://handbook.gitlab.com/handbook/product-development/how-we-work/product-development-flow/feature-flag-lifecycle/#including-a-feature-behind-feature-flag-in-the-final-release).
    * Merging by the milestone code cut-off (Friday) **does not guarantee** that the feature will be in the release. See [release timelines](https://handbook.gitlab.com/handbook/engineering/releases/monthly-releases/#timelines).
    * Individual [release post entries](https://handbook.gitlab.com/handbook/marketing/blog/release-posts/#contribution-instructions) merged for all relevant issues.
    * By end of the day, milestone `%x.y` is expired.
  * **On or around Wednesday, the day before the release date** : 
    * [Group Retrospective issues](https://handbook.gitlab.com/handbook/engineering/careers/management/group-retrospectives/) are updated with shipped and missed deliverables and team-members are tagged in the discussion.
  * **Wednesday, the day before the release date** : 
    * [Milestone Cleanup](https://handbook.gitlab.com/handbook/engineering/workflow/#milestone-cleanup) runs on the schedule at [Milestone cleanup schedule](https://handbook.gitlab.com/handbook/engineering/workflow/#milestone-cleanup-schedule).
  * **Third Thursday of the release month** : _**Release Day!**_ 🚀 
  ...
  * **On or around the Wednesday immediately following the release day** : 
    * [Product plans](https://handbook.gitlab.com/handbook/product/product-processes/#managing-your-product-direction) are updated to reflect previous and current releases, including category epics and direction pages.
...
Refer to [release post content reviews](https://handbook.gitlab.com/handbook/marketing/blog/release-posts/#content-reviews) for additional deadlines.
Note that deployments to GitLab.com are more frequent than monthly major/minor releases. See [auto deploy transition](https://gitlab.com/gitlab-org/release/docs/blob/21cbd409dd5f157fe252f254f3e897f01908abe2/general/deploy/auto-deploy-transition.md#transition) guidance for details.
## Kickoff[](https://handbook.gitlab.com/handbook/engineering/workflow/#kickoff)
At the beginning of each release, we have a kickoff meeting, publicly livestreamed to YouTube. In the call, the Product Development team (PMs, Product Designers, and Engineers) communicate with the rest of the organization which issues are in scope for the upcoming release. The call is structured by [product area](https://handbook.gitlab.com/handbook/product/categories/#devops-stages) with each PM leading their part of the call.
...
## Milestone Cleanup[](https://handbook.gitlab.com/handbook/engineering/workflow/#milestone-cleanup)
...
Milestones closure is in the remit of [the Delivery team](https://handbook.gitlab.com/handbook/engineering/infrastructure-platforms/gitlab-delivery/delivery/). At any point in time a release might need to be created for an active milestone,and once that is no longer the case, the Delivery team closes the milestone.
### Milestone cleanup schedule[](https://handbook.gitlab.com/handbook/engineering/workflow/#milestone-cleanup-schedule)
The milestone cleanup will happen on the milestone due date.
These actions will be applied to open issues:
  * Open issues and merge requests will be moved to the next milestone, and labeled with `~"missed:x.y"`.
  * `~"missed-deliverable"` will also be added whenever `~"Deliverable"` is presented.


Milestones are closed when the Delivery team no longer needs to create a backport release for a specific milestone.
## Use Group Labels and Group Milestones[](https://handbook.gitlab.com/handbook/engineering/workflow/#use-group-labels-and-group-milestones)
...
## Technical debt[](https://handbook.gitlab.com/handbook/engineering/workflow/#technical-debt)
...
Therefore technical debt may be accrued but must be:
  ...
  4. assigned a [backlog label](https://gitlab.com/groups/gitlab-org/-/epics/18639), `~"backlog::prospective"` if it is expected to be worked on in the next 18 months, `~"backlog::no-commitment"` otherwise.
...
Technical debt is prioritized like [other technical decisions](https://handbook.gitlab.com/handbook/engineering/development/principles/#prioritizing-technical-decisions) in [product groups](https://handbook.gitlab.com/handbook/company/structure/#product-groups) by [product management](https://handbook.gitlab.com/handbook/product/product-processes/cross-functional-prioritization/).
For technical debt which might span, or fall in gaps between groups they should be brought up for a [globally optimized](https://handbook.gitlab.com/handbook/values/#efficiency-for-the-right-group) prioritization in [retrospectives](https://handbook.gitlab.com/handbook/engineering/careers/management/group-retrospectives/) or directly with the appropriate member of the [Product Leadership team](https://handbook.gitlab.com/handbook/product/product-leaders/product-leadership/). Additional avenues for addressing technical debt outside of product groups are [Strategic Priority Codes](https://handbook.gitlab.com/handbook/engineering/workflow/strategic-priority-codes/) and [working groups](https://handbook.gitlab.com/handbook/company/working-groups/).
## Deferred UX[](https://handbook.gitlab.com/handbook/engineering/workflow/#deferred-ux)
Sometimes there is an intentional decision to deviate from the agreed-upon [MVC](https://handbook.gitlab.com/handbook/product/product-principles/#the-minimal-valuable-change-mvc), which sacrifices the user experience. When this occurs, the Product Designer creates a follow-up issue and labels it `Deferred UX` to address the UX gap in subsequent releases.
For the same reasons as technical debt, we don’t want Deferred UX to grow faster than our code base.
These issues are prioritized like [other technical decisions](https://handbook.gitlab.com/handbook/engineering/development/principles/#prioritizing-technical-decisions) in [product groups](https://handbook.gitlab.com/handbook/company/structure/#product-groups) by [product management](https://handbook.gitlab.com/handbook/product/product-processes/cross-functional-prioritization/).
As with [technical debt](https://handbook.gitlab.com/handbook/engineering/workflow/#technical-debt), Deferred UX should be brought up for [globally optimized](https://handbook.gitlab.com/handbook/values/#efficiency-for-the-right-group) prioritization in [retrospectives](https://handbook.gitlab.com/handbook/engineering/careers/management/group-retrospectives/) or directly with the appropriate member of the [Product Leadership team](https://handbook.gitlab.com/handbook/product/product-leaders/product-leadership/).
## UI polish[](https://handbook.gitlab.com/handbook/engineering/workflow/#ui-polish)
...
### Examples of UI polish[](https://handbook.gitlab.com/handbook/engineering/workflow/#examples-of-ui-polish)
...
### What is not UI polish[](https://handbook.gitlab.com/handbook/engineering/workflow/#what-is-not-ui-polish)
...
## Monitor Merge Request Trends[](https://handbook.gitlab.com/handbook/engineering/workflow/#monitor-merge-request-trends)
Open merge requests sometimes become idle (not updated by a human in more than a month). Once a month, engineering managers will receive an [`Merge requests requiring attention triage issue`](https://handbook.gitlab.com/handbook/engineering/infrastructure-platforms/developer-experience/triage-operations/#group-level-merge-requests-that-may-need-attention) that includes all (non-WIP/Draft) MRs for their group and use it to determine if any action should be taken (such as nudging the author/reviewer/maintainer). This assists in getting merge requests merged in a reasonable amount of time which we track with the [Open MR Review Time (OMRT)](https://handbook.gitlab.com/handbook/engineering/development/performance-indicators/#open-mr-review-time-omrt) and [Open MR Age (OMA)](https://handbook.gitlab.com/handbook/engineering/development/performance-indicators/#open-mr-age-oma) performance indicators.
...
## Security is everyone’s responsibility[](https://handbook.gitlab.com/handbook/engineering/workflow/#security-is-everyones-responsibility)
...
Being able to start the security review process earlier in the software development lifecycle means we will catch vulnerabilities earlier, and mitigate identified vulnerabilities before the code is merged. You should know when and how to proactively [seek an Application Security Review](https://handbook.gitlab.com/handbook/security/product-security/security-platforms-architecture/application-security/appsec-reviews/). You should also be familiar with our [Secure Coding Guidelines](https://docs.gitlab.com/ee/development/secure_coding_guidelines.html).
...
## Rapid Engineering Response[](https://handbook.gitlab.com/handbook/engineering/workflow/#rapid-engineering-response)
...
### Scope[](https://handbook.gitlab.com/handbook/engineering/workflow/#scope)
Not everything is urgent. See below for a non-exclusive list of things that are in-scope and not in-scope. As always, use your experience and judgment, and communicate with others.
  * In Scope 
    ...
    * High severity (severity::1/priority::1) security issues. Refer to [security severity and priority](https://handbook.gitlab.com/handbook/security/#severity-and-priority-labels-on-security-issues).
  ...
  * Not In Scope 
    * An operational issue of GitLab.com or a self managed customer environment. This falls under the [on-call](https://handbook.gitlab.com/handbook/engineering/on-call/) process.
...
### Process[](https://handbook.gitlab.com/handbook/engineering/workflow/#process)
  1. Person requesting Rapid Engineering Response creates an issue supplying all known information and applies [priority and severity](https://gitlab.com/gitlab-org/gitlab-foss/-/blob/master/doc/development/contributing/issue_workflow.md#priority-labels) (or [security severity and priority](https://handbook.gitlab.com/handbook/security/#severity-and-priority-labels-on-security-issues)) to the best of their ability.
  2. Person requesting Rapid Engineering Response raises the issue to their own manager and the [subject matter domain](https://handbook.gitlab.com/handbook/product/categories/) engineering manager (or the delegation if OOO). 
    1. In case a specific group cannot be determined, raise the issue to the Director of Engineering (or the delegation if OOO) of the [section](https://handbook.gitlab.com/handbook/product/categories/).
  ...
  4. Adjust [priority and severity](https://gitlab.com/gitlab-org/gitlab-foss/-/blob/master/doc/development/contributing/issue_workflow.md#priority-labels) or [security severity and priority](https://handbook.gitlab.com/handbook/security/#severity-and-priority-labels-on-security-issues) if necessary, and work collaboratively on the determined resolution.


## Performance Refinement[](https://handbook.gitlab.com/handbook/engineering/workflow/#performance-refinement)
A bi-weekly performance refinement session is held by the Development and QE teams jointly to raise awareness and foster wider collaboration about high-impact performance issues. A high impact issue has a direct measurable impact on GitLab.com [service levels or error budgets](https://gitlab.com/gitlab-com/gl-infra/readiness/-/tree/master/library/service-levels-error-budgets).
### Scope[](https://handbook.gitlab.com/handbook/engineering/workflow/#scope-1)
...
### Process[](https://handbook.gitlab.com/handbook/engineering/workflow/#process-1)
  1. To participate in the bi-weekly refinement, ask your engineering director to forward the invite of the _Performance Refinement_ meeting which is at 15:00 UTC every other Thursday. Here is the [meeting agenda](https://docs.google.com/document/d/1icG6yrW2oebXz8iXvgfM5JjtMqpsDBCn1v3_VO2ghS0/edit).
  2. To nominate issues to the board: 
    1. Assign [a performance severity](https://handbook.gitlab.com/handbook/product-development/how-we-work/issue-triage/#severity) on the issue to help asses the priority assignment for the refinement session.
...
### Roles and Responsibilities[](https://handbook.gitlab.com/handbook/engineering/workflow/#roles-and-responsibilities)
#### Infrastructure[](https://handbook.gitlab.com/handbook/engineering/workflow/#infrastructure)
  1. Nominate issues by adding `Infradev` label.
  2. Assess [Severity](https://handbook.gitlab.com/handbook/product-development/how-we-work/issue-triage/#severity-slos) and `Priority` and apply the corresponding label as appropriate.
  3. Provide [as much information as possible](https://handbook.gitlab.com/handbook/engineering/workflow/#a-guide-to-creating-effective-infradev-issues) to assist development engineering troubleshooting.


#### Development[](https://handbook.gitlab.com/handbook/engineering/workflow/#development)
  1. Development directors are responsible for [triaging Infradev issues](https://handbook.gitlab.com/handbook/engineering/development/#daily-duties-for-engineering-directors) regularly by following the [triage process](https://handbook.gitlab.com/handbook/engineering/workflow/#triage-process) below.
  2. Development managers are encouraged to [triage issues](https://handbook.gitlab.com/handbook/engineering/workflow/#triage-process) regularly as well.
...
#### Product Management[](https://handbook.gitlab.com/handbook/engineering/workflow/#product-management)
...
### Triage Process[](https://handbook.gitlab.com/handbook/engineering/workflow/#triage-process)
...
Issues are nominated to the board through the inclusion of the label `infradev` and will appear on the [infradev board](https://gitlab.com/groups/gitlab-org/-/boards/1193197?label_name\[\]=infradev).
  ...
  2. Review the [severity](https://handbook.gitlab.com/handbook/product-development/how-we-work/issue-triage/#severity) on the issue to validate appropriate prioritization.
...
Infradev issues are also shown in the monthly [Error Budget Report](https://handbook.gitlab.com/handbook/engineering/error-budgets/#budget-reporting).
### A Guide to Creating Effective Infradev Issues[](https://handbook.gitlab.com/handbook/engineering/workflow/#a-guide-to-creating-effective-infradev-issues)
Triage of infradev Issues is desired to occur asynchronously. These points below with endure that your infradev issues gain maximum traction.
  ...
  2. **Clearly state the scope of the problem, and how it affects GitLab SaaS Platforms**. Examples could include: 
    ...
    2. Saturation issues: the problem could leave to increased saturation, latency issues due to resource over-utilization. [example](https://gitlab.com/gitlab-org/charts/gitlab/-/issues/2458)
    3. Service-level degradation: the problem is causing our service-level monitoring to degrade, impacting the overall SLA of GitLab.com and potentially leaving to SLA violations. [example](https://gitlab.com/gitlab-org/gitlab/-/issues/292732)
    4. Alerts not resulting in incidents: alerts that are triggered but do not result in actionable incidents for the Tier 1 EOC (SRE on call), yet require action by the team owning the service. Examples include [noisy alerts](https://gitlab.com/gitlab-com/gl-infra/production-engineering/-/issues/28218#note_3035628793), alerts that need tuning, alerts that signal a growing problem that is not yet an incident but may become one, or alerts that [no longer need to exist](https://gitlab.com/gitlab-org/gitlab/-/issues/292732). The team owning the service that is triggering the alert is responsible for determining the correct outcome and taking action (proactive action, tune, or remove). This reduces alerting noise and distraction for the Tier 1 EOC to only include alerts that represent production incidents while ensuring all alerts are prioritized and addressed appropriately.
    5. Problems which extend the time to diagnosis of incidents: for example, issues which degrade the observability of GitLab.com, swallow user-impacting errors or logs, etc. These could lead to incidents taking much longer to clear, and impacting availability. [example](https://gitlab.com/gitlab-com/gl-infra/infrastructure/-/issues/10933#note_464394760)
    6. Deficiencies in our public APIs which lead to customers compensating by generating substantially more traffic to get the required results. [example](https://gitlab.com/gitlab-org/gitlab/-/issues/232887)
  ...
  6. **Ensure a realistic severity is applied** : review the [availability severity label guidelines](https://handbook.gitlab.com/handbook/product-development/how-we-work/issue-triage/#availability) and ensure that applied severity matches. Always ensure all issues have a severity, even if you are unsure.
  7. **If possible, include ownership labels** for more effective triage. The [product categories](https://handbook.gitlab.com/handbook/product/categories/) can help determine the appropriate stage group to assign the issue to.
  ...
  11. **Provide a method for validating that the original issue still exists**
    ...
    4. Ideally, provide a link to a Grafana query or an ELK query and clear instructions on how to interpret the results to determine whether the problem is still occurring. [Check the “Verfication” section in this issue as an example of this](https://gitlab.com/gitlab-org/gitaly/-/issues/3670#verification).
...
#####  [Duo-First Development](https://handbook.gitlab.com/handbook/engineering/workflow/duo-first-development/)
Overview Development Standards define how we work as engineers at GitLab. These standards establish …
...
#####  [Engineering Demo Process](https://handbook.gitlab.com/handbook/engineering/workflow/demos/)
Occasionally, it is useful to set up a demo on a regular cadence to ensure cross-functional …
#####  [Engineering Hiring](https://handbook.gitlab.com/handbook/engineering/workflow/hiring/)
Overview Hiring is a cornerstone of success for our engineering organization, contributing to our …
#####  [Fast Boot](https://handbook.gitlab.com/handbook/engineering/workflow/fast-boot/)
...
#####  [Guidelines for automation and access tokens](https://handbook.gitlab.com/handbook/engineering/workflow/automation/)
Guidelines for automation with project/group tokens or service accounts
#####  [Innovation at GitLab](https://handbook.gitlab.com/handbook/engineering/workflow/engineering-innovation/)
...
#####  [Onboarding](https://handbook.gitlab.com/handbook/engineering/workflow/development-onboarding/)
#####  [Processes](https://handbook.gitlab.com/handbook/engineering/workflow/development-processes/)
...
#####  [Unplanned Upgrade Stop Workflow](https://handbook.gitlab.com/handbook/engineering/workflow/unplanned-upgrade-stop/)
...
[ ](https://gitlab.com/gitlab-com/content-sites/handbook/blob/main/content/handbook/engineering/workflow/_index.md) - [ ](https://gitlab.com/-/ide/project/gitlab-com/content-sites/handbook/edit/main/-/content/handbook/engineering/workflow/_index.md) - please [contribute](https://handbook.gitlab.com/handbook/about/contributing/). [ ![Creative Commons License](https://i.creativecommons.org/l/by-sa/4.0/80x15.png)](https://creativecommons.org/licenses/by-sa/4.0/)