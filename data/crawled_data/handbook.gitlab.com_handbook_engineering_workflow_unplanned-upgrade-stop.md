...
## What is unplanned upgrade stop?[](https://handbook.gitlab.com/handbook/engineering/workflow/unplanned-upgrade-stop/#what-is-unplanned-upgrade-stop)
...
### Unplanned upgrade severity[](https://handbook.gitlab.com/handbook/engineering/workflow/unplanned-upgrade-stop/#unplanned-upgrade-severity)
...
## Unplanned upgrade stop Detection[](https://handbook.gitlab.com/handbook/engineering/workflow/unplanned-upgrade-stop/#unplanned-upgrade-stop-detection)
_(DRI: Support team or the team/individual detecting the issue)_
  ...
  2. Raise [Unplanned upgrade stop RCA issue](https://gitlab.com/gitlab-org/gitlab/-/blob/master/.gitlab/issue_templates/rca_upgrade_stop.md) with brief summary, customer impact and detection
  3. Assign the RCA issue to PM or EM of Engineering team owning the bug
  4. Post in the [#f_gitlab_upgrades](https://gitlab.enterprise.slack.com/archives/C04FXB5CP08) Slack channel


## Unplanned upgrade stop Resolution[](https://handbook.gitlab.com/handbook/engineering/workflow/unplanned-upgrade-stop/#unplanned-upgrade-stop-resolution)
_(DRI: To be identified by the Engineering Manager (EM) of the team owning the bug)_
  ...
  2. DRI to work on resolving the bug as quickly as possible. The first priority is to find a fix.
  3. DRI to update the bug with workaround to unblock the customer if applicable.
  4. DRI to identify if fix can be backported. If backporting is not possible, collaborate with the Distribution team to update the upgrade path and [retroactively add the unplanned stop](https://docs.gitlab.com/ee/development/avoiding_required_stops.html#retroactively-adding-required-stops) for immediate resolution.
  5. EM to identify DRI for performing Unplanned upgrade stop RCA.
  6. RCA DRI performs Root Cause Analysis and raises corrective actions.
  7. RCA DRI collaborate with Test Platform on test coverage analysis and raise corrective actions.
  8. EM to prioritize corrective actions by following [GitLab Priority label](https://handbook.gitlab.com/handbook/product-development/how-we-work/issue-triage/#priority) guidelines for RCA severity.


## Metrics[](https://handbook.gitlab.com/handbook/engineering/workflow/unplanned-upgrade-stop/#metrics)
Root Cause Analysis issues help to collect metrics for unplanned upgrade stops and bring visibility into bugs severity and impact. List of all RCA for upgrades can be found [in the GitLab project’s issues by the `~RCA` and `~upgrades` label](https://gitlab.com/gitlab-org/gitlab/-/issues/?sort=created_date&state=all&label_name%5B%5D=RCA&label_name%5B%5D=upgrades&first_page_size=20).
## Links[](https://handbook.gitlab.com/handbook/engineering/workflow/unplanned-upgrade-stop/#links)
...
[ ](https://gitlab.com/gitlab-com/content-sites/handbook/blob/main/content/handbook/engineering/workflow/unplanned-upgrade-stop.md) - [ ](https://gitlab.com/-/ide/project/gitlab-com/content-sites/handbook/edit/main/-/content/handbook/engineering/workflow/unplanned-upgrade-stop.md) - please [contribute](https://handbook.gitlab.com/handbook/about/contributing/). [ ![Creative Commons License](https://i.creativecommons.org/l/by-sa/4.0/80x15.png)](https://creativecommons.org/licenses/by-sa/4.0/)